...
class OperationsExpeditionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Expedition
        fields = ("id", "name", "slug", "destination", "departure_location", "starts_at", "ends_at", "duration_days", "capacity", "occupied_slots", "available_slots", "price_per_person_cents", "deposit_cents", "balance_due_days_before", "status", "summary", "created_at", "updated_at")
...
class OperationsReservationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Reservation
        fields = ("id", "status", "participant_count", "unit_price_cents", "total_price_cents", "deposit_cents", "payment_plan", "paid_amount_cents", "remaining_balance_cents", "balance_due_at", "held_until", "beverage_preferences", "created_at", "customer", "expedition", "participants", "payments", "events")
    ...
    def get_participants(self, obj):
        return [{"id": str(item.id), "name": item.full_name, "phone": item.phone, "onboarding_status": item.onboarding_status} for item in obj.participants.all()]
